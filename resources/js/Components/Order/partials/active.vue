...
<template>
	...
	<div>
		...
		<div class="columns is-multiline"> <!-- Columns wrapper tag open -->
			...
			<div class="column is-6" v-for="order in searchQuery.length  > 1  ? loadOrderSearch : loadOrderConfirmed"> <!-- column tag open-->
		...
		<div class="card" v-if="loadOrderConfirmed.length <= 0">
...
export default {
	...
	methods: {
		...mapActions(['fetchConfirmOrder', 'clearOrderNotification', 'searchOrder', 'updateOrderStatus']),
		// Local method
		refresh() {
			this.spin = true
			this.fetchConfirmOrder().then(() => this.spin = false)
			this.searchQuery = ""
			this.searchOrder(this.searchQuery)
		},

		paginationHandler(uri) {
			this.fetchConfirmOrder(uri)
		},
	...
	computed: {
		...mapGetters(['loadOrderConfirmed', 'loadOrderLoader', 'loadOrderNotification', 'loadOrderPagination', 'loadOrderSearch', 'loadAuthUser', 'loadAuthRole']),
