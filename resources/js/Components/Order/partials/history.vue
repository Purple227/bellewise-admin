...
<div class="pageloader purple-bg" v-bind:class="{ 'is-active': loadOrderLoader }"><span class="title"> Bellewise loading </span></div>
		...
		<div class="columns is-multiline"> <!-- Columns wrapper tag open -->
			...
			<div class="column is-6" v-for="order in searchQuery.length  > 1  ? loadOrderSearch : loadOrderHistory"> <!-- column tag open-->
...
export default {
	...
	methods: {
		...mapActions(['fetchHistoryOrder', 'clearOrderNotification', 'searchOrder', 'updateOrderStatus']),
		// Local method
		refresh() {
			this.spin = true
	...
	computed: {
		...mapGetters(['loadOrderHistory', 'loadOrderLoader', 'loadOrderNotification', 'loadOrderPagination', 'loadOrderSearch', 'loadAuthUser', 'loadAuthRole']),
