...
<template>
  ...
  <div class="" > <!-- Root element tag open -->
    ...
    <div class="notification purple-bg-light is-bold has-text-black" v-if="loadConfigNotification">
      Task Succeesful
    </div>
...
export default {
  ...
  mounted() {
    this.bulmaCalendar()
    this.setId()
    this.clearConfigNotification()
  },


  methods: {

    ...mapActions(['restaurantConfigSetting','fetchRestaurantConfig', 'editRestaurantConfig', 'clearConfigNotification']),
    ...
    submitForm() {
      ...
      let id = this.$route.params.id
      this.restaurantConfigSetting({data, id}).then(() => this.setId() )
...
computed: {
  ...mapGetters(['loadConfigProgress', 'loadConfigNotification', 'loadRestaurantConfig', 'loadConfigLoader', 'loadConfigNotification']),
},
