...
<template>
	...
	<div> <!-- Root element tag open -->
		...
		<div class="card"> <!-- Card tag open -->
			...
			<div class="card-content table-container"> <!-- Card content tag open -->
				...
				<div class="center"> <!-- Center table tag -->
					...
					<table class="table is-bordered is-striped is-hoverable" v-if="loadBlockedMenus.length >= 1"> <!-- Table tag open -->
						...
						<tbody>
							...
							<tr v-for="(menu, index) in searchQuery.length  > 1  ? loadMenuSearch : loadBlockedMenus " :key="index">
				...
				<!-- Pagination section -->
				<div class="buttons has-addons is-centered" v-if="loadBlockedMenus.length >= 1">
		...
		<div class="card" v-if="loadBlockedMenus.length <= 0">
			<div class="card-content">
				<div class="content is-bold has-text-centered subtitle">

					<span class="fa"> No blocked menu found. </span>
...
export default {
	...
	methods: {
		...mapActions(['fetchBlockedMenus', 'clearMenuNotification', 'destroyMenuData', 'searchMenuDatas', 'updateMenuStatus']),
    // Local method
    
    refresh() {
    	this.spin = true
    	this.fetchBlockedMenus().then(() => this.spin = false)
    	this.searchQuery = ""
    ...
    async deleteData(id, index) {
    	...
    	this.fetchBlockedMenus()
    	this.showModal = false
    },

    getMenus() {
    	let id = this.$route.params.id
    	this.fetchBlockedMenus({id})
    },

    paginationHandler(uri) {
    	this.fetchBlockedMenus({uri})
    },
...
computed: {
	...mapGetters(['loadBlockedMenus', 'loadMenuLoader', 'loadMenuNotification', 'loadMenuPagination', 'loadMenuSearch', 'loadAuthRole', 'loadAuthUser']),
