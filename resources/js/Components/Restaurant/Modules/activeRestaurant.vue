     ...
     <div class="card-content table-container"> <!-- Card content tag open -->
      ...
      <table class="table is-bordered is-striped is-hoverable" v-if="loadActiveRestaurants.length >= 1"> <!-- Table tag open -->
        ...
        <tbody>
            ...
            <tr v-for="(restaurant, index) in loadActiveRestaurants" :key="index">
...
<div class="card" v-if="loadActiveRestaurants.length <= 0">
  ...
  <span class="fa"> No restaurant active. </span>
...
export default {
  ...
  methods: {
    ...mapActions(['fetchActiveRestaurants', 'clearRestaurantNotification', 'destroyRestaurantData', 'updateRestaurantStatus', 'loadAuthUser', 'loadAuthRole']),
    // Local method
    refresh() {
      this.spin = true
      this.fetchActiveRestaurants().then(() => this.spin = false)
    },
  ...
  computed: {
    ...mapGetters(['loadActiveRestaurants', 'loadRestaurantLoader', 'loadRestaurantNotification', 'loadRestaurantPagination', 'loadAuthUser', 'loadAuthRole']),
